...
Un BSC es un canal discreto sin memoria con alfabeto de entrada X{0, 1} y salida Y {0,1}. Su comportamiento probabilístico está definido por la matriz de transición:

P(Y|X) =  P(Y=0|X=0)=1-p  P(Y=1|X=0)=p 
...
Mide la incertidumbre o promedio de información recibido por símbolo:

H(Y) = - P(Y=0)\log_2 P(Y=0) - P(Y=1)\log_2 P(Y=1)  bits/símbolo
...
H(Y|X) = H(p) = -p \log_2(p) - (1-p) \log_2(1-p) bits/símbolo
...
* **`atender_cliente(cliente, direccion)`:** Maneja la sesión TCP con cada cliente en un hilo independiente (`threading`). Para cada bit recibido, genera un valor pseudoaleatorio mediante `random.random()`. Si dicho valor es menor que `P_ERROR`, invierte el bit, de lo contrario, lo transmite intacto.
...
* **`ejecutar_fase1(sock)`:**
  ...
  - Compara las tramas recibidas bit a bit y calcula BER = errores/N
